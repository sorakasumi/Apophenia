...
var hexPaper = {
    ...
    build: function(screenSize) {
        ...
        for (i = 0; i < gridSize.height; i++) {
            ...
            for (i2 = 0; i2 < gridSize.width; i2++) {
                var point = new Point(
                    trueBounds.point + new Point(i2, i) * hexSize
                );
                point.x += alternate;
    ...
    cleanJunk: function() {
        ...
        for (i = 0; i < hexList.length; i++) {
            ...
            if (
                pos.x > bounds.width - buffer ||
                pos.y > bounds.height - buffer ||
                pos.y < 0 + buffer ||
                pos.x < 0 + buffer
            ) {
    ...
    buildTest: function(testSize) {
        ...
        var squareRotateBounds = new Path.Rectangle(
            view.center,
            new Size(pythagAB(testSize))
        );
